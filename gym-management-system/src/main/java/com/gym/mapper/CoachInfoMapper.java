...
public interface CoachInfoMapper {
    @Insert("INSERT INTO coach_info (user_id, name, gender, birth_date, phone, specialty, certification, introduction) VALUES (#{user.id}, #{name}, #{gender}, #{birthDate}, #{phone}, #{specialty}, #{certification}, #{introduction})")
    ...
    @Update("UPDATE coach_info SET name = #{name}, gender = #{gender}, birth_date = #{birthDate}, phone = #{phone}, specialty = #{specialty}, certification = #{certification}, introduction = #{introduction} WHERE user_id = #{user.id}")
